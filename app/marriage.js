/* ============================================================================
   Marriage Map — a globe coloured by a chosen marriage "perspective", with a
   per-country profile and a scrubbable history-of-marriage timeline.
   Data: data/history.js (METRICS, EVENTS, TRENDS) + data/marriage-data.js.
   Engine: globe.gl (bundled). Forked from Jewish History Explorer.
   ========================================================================== */
'use strict';
...
const EVENTS  = (window.MARRIAGE_EVENTS || []).slice().sort((a, b) => a.year - b.year);
...
const state = { metric: 'divorce', stepIdx: N - 1, hovered: null, selected: null, selectedEvent: null, playing: false, playDir: 1, flat: false };
...
function showDetail(iso, feat) {
  ...
  document.getElementById('detailSub').textContent = c ? 'Marriage profile' : 'No data available';
  const box = document.getElementById('detailMetrics');
  if (!c) { box.innerHTML = '<div class="tt-nd" style="padding:8px 4px">No marriage data for this country yet.</div>'; }
  else {
...
function updateTimelineState() {
  ...
  const f = trendAt('fertility', y); if (f != null) bits.push('≈ ' + f.toFixed(1) + ' children/woman');
  const am = trendAt('ageFirstMarriageFemale', y); if (am != null) bits.push('women marry ~' + Math.round(am));
...
function setFlat(flat) {
  ...
  if (flat) { buildFlatMap(); updateFlatColors(); updateFlatPins(); try { if (!localStorage.getItem('mm_seen_flat') && document.getElementById('tutorial').classList.contains('hidden')) showFlatTip(); } catch (e) {} }
...
function closeTutorial() { const t = document.getElementById('tutorial'); if (t.classList.contains('hidden')) return; t.classList.add('hidden'); try { localStorage.setItem('mm_seen_tutorial', '1'); } catch (e) {} }
...
function closeFlatTip() { const t = document.getElementById('flatTip'); if (t.classList.contains('hidden')) return; t.classList.add('hidden'); try { localStorage.setItem('mm_seen_flat', '1'); } catch (e) {} }
...
function boot() {
  ...
  try { if (!localStorage.getItem('mm_seen_tutorial')) showTutorial(); } catch (e) {}
